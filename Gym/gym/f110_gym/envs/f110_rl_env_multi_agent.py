...
class RLF110MultiAgentEnv(F110Env): 
    def __init__(self, random_init=False, visualize=False, use_lidar=False, single_trained_model_path = None, opp_idx=1, **kwargs):
        super().__init__(**kwargs)

        self.opp_idx = opp_idx
        if single_trained_model_path is not None:
            self.model = SAC.load(single_trained_model_path, device="cpu")
        else:
        ...
        ## Hardcoding stuff for two agents for now -> Look into generalizing for n agents later

        ## Rand theta is initialized randomly and reduced by 0.8 at each agent
        if random_init:
        ...
        if self.num_agents == 1:
            ...
            self.positions = [self.sim.agents[i].state[:2] for i in range(self.num_agents)]
            self.rand_thetas = ([rand_theta - 0.8*i for i in range(self.num_agents)])
            self.rand_thetas.reverse()
            self.thetas = [self.track.find_theta(self.positions[i], self.rand_thetas[i]) for i in range(self.num_agents)]
            self.next_waypoints = [int(np.floor(self.thetas[i])) + 1 for i in range(self.num_agents)]
            self.times = [40*np.ones((int(self.track.track_length)+1,)) for i in range(self.num_agents)]
            self.last_visited_times = [-1*np.ones((int(self.track.track_length)+1,)) for i in range(self.num_agents)]
    ...
    def step(self, action_ego):
        ...
        if action_ego.shape == (2,):
            action_ego = action_ego.reshape((1, 2))
        #print(action.shape)

        norm_action = action_ego.copy()
        action_ego = self.denorm_action(action_ego)

        self.update_thetas()

        ## Getting the opponent state by performing a step with no action

        empty_action = np.zeros((self.num_agents, 2))
        obs, _, done, info = super(RLF110MultiAgentEnv, self).step(action=empty_action)

        # Get the observation in desired format for running the SAC model (Hardcoding for now since frenet trajectory is the one being used)
        obs_opp, _,_,_ = self.get_frenet_traj_obs(obs, 0, norm_action, empty_action, is_ego=False)
        action_opp, _ = self.model.predict(obs_opp, deterministic=True)
        action_opp = action_opp.reshape((1,2))
        action_opp = self.denorm_action(action_opp)

        ## Appending the two actions together and getting the actual reward and observation
        # print(action_ego.shape, action_opp.shape)
        action = np.concatenate((action_ego, action_opp), axis=0).reshape((self.num_agents, 2))
        




        obs, _, done, info = super(RLF110MultiAgentEnv, self).step(action=action)


        

        reward = self.get_reward(rew_type='adv_opp')
    ...
    def reset(self, poses=-1, seed=None):
        ...
        for i in range(self.num_agents):
            ...
            rand_theta += 0.8
            # self.start_param = rand_theta
        # print(self.start_pose, self.start_pose.shape)
        ...
        def get_next_waypoint(idx):
            waypoint = int(np.floor(self.thetas[idx])) + 1
            if waypoint < 0:
                waypoint += int(self.track.track_length)
            return waypoint
        ...
        if self.num_agents == 1:
            ...
            self.positions = [self.sim.agents[i].state[:2] for i in range(self.num_agents)]
            self.rand_thetas = ([self.start_param - 0.8*i for i in range(self.num_agents)])
            self.rand_thetas.reverse()
            self.thetas = [self.track.find_theta(self.positions[i], self.rand_thetas[i]) for i in range(self.num_agents)]
            self.next_waypoints = [get_next_waypoint(i) for i in range(self.num_agents)]
            self.times = [40*np.ones((int(self.track.track_length)+1,)) for i in range(self.num_agents)]
            self.last_visited_times = [-1*np.ones((int(self.track.track_length)+1,)) for i in range(self.num_agents)]
    ...
    def render(self, mode='human', close=False):
        return super(RLF110MultiAgentEnv, self).render(mode=mode)
    ...
    def get_frenet_obs(self, obs, reward, action, is_ego=True):
        idx = self.ego_idx if is_ego else self.opp_idx
        obs = self.obs_preselection(obs)
        car_position = self.sim.agents[idx].state[:2]
        car_angle = obs['poses_theta'][0]%(2*np.pi)

        # following line can be used if lidar but with less than 1080 scans is wanted
        filtered_scans = self.filter_scans(obs['scans'][idx])
        
        new_obs = {'scans':np.array(filtered_scans)} #'scans': obs['scans']}
        #new_obs['param'] = [self.theta]
        new_obs['deviation'] = [np.linalg.norm(car_position-self.track.get_coordinate(self.thetas[idx]))]
        rel_head = self.get_rel_head(obs, idx)
        new_obs['rel_heading'] = [rel_head]
        new_obs['longitudinal_vel'] = [obs['linear_vels_x'][idx]]
        new_obs['later_vel'] = [obs['linear_vels_y'][idx]]
        new_obs['yaw_rate'] = [obs['ang_vels_z'][idx]]
    ...
    def get_frenet_traj_obs(self, obs, reward, norm_action, action, is_ego=True):
        """
        ...
        filtered_scans = self.filter_scans(obs['scans'][idx])
        car_position = self.sim.agents[idx].state[:2]
        car_angle = obs['poses_theta'][idx]%(2*np.pi)
        th_on_traj = self.track.trajectory.find_theta(car_position, self.thetas[idx]+self.track.delta_traj)
        rel_head = self.get_rel_head(obs, idx)
        slip_angle = self.sim.agents[idx].state[6]
    ...
    def update_thetas(self):
        ...
        """
        # car_position = [self.sim.agents[self.ego_idx].state[:2]]
        car_positions = [self.sim.agents[i].state[:2] for i in range(self.num_agents)]
        self.prev_theta = self.thetas[self.ego_idx]
        self.thetas = [self.track.find_theta(car_positions[i], self.thetas[i]) for i in range(self.num_agents)]

    def get_rel_head(self, obs, idx):
        """
        Obtain heading of the car relative to the track
        """

        angle = self.track.get_angle(self.thetas[idx])
        if angle < 0:
            print("Really? angle should always be > 0")
            angle+=2*np.pi
        if not np.isnan(obs['poses_theta'][idx]):
            car_angle = obs['poses_theta'][idx]%(2*np.pi)
        else:
            car_angle = 0
